...
//     const GstMetaInfo* info = GST_AD_BATCH_META_INFO;
    ...
    {
        float prob = (double)( rand() % 1000 )/1000;
        std::string label = labels[index];
        double x1 = (double)( rand() % 3 + 1 )/10;	// 0.1~0.3
        double x2 = (double)( rand() % 3 + 7 )/10;	// 0.7~0.9
        double y1 = (double)( rand() % 3 + 1 )/10;	// 0.1~0.3
        double y2 = (double)( rand() % 3 + 7 )/10;	// 0.7~0.9
        
        std::shared_ptr<ROI> obj_box = adroi_new_box("sample-engine", "", prob, x1, y1, x2, y2);
        obj_box->add_classification("sample-engine", "", prob, label, 0);
...
//     g_object_set (drawer, "print", true, NULL);
